package es.juanc.katas.wordwarp

import es.juanc.katas.UnitFlatSpec

class WordWarpSpec extends UnitFlatSpec {

  "WordWrap" must "return empty string is no text is specified" in {
    "".wordWrap(10) should be ("")
  }

  it must "work with a single word" in {
  ...
  it should "return wrapped text with small sample" in {
    ...
    input.wordWrap(4) shouldBe expected
  }
